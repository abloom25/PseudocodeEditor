export interface ConvertedArrayData {
  bounds: number[][];
  is2D: boolean;
  elementType: string;
...
function convertArraysData(
  rawArrays: Record<string, { dims: { lower: number; upper: number }[]; data: unknown[] }>,
  variableTypes: Record<string, string>
): Record<string, ConvertedArrayData> {
  const result: Record<string, ConvertedArrayData> = {};
